...
function ReportModal({ post, onClose }) {
    ...
    const submit = async (event) => {
        ...
        setSubmitting(true);
        try {
            await axios.post(`/api/board/${post.boardnum}/report`, { reason, detail });
            alert('신고가 접수되었습니다.');
            onClose();
        } catch (error) {
            if (error.response?.status === 409) alert('이미 신고한 게시글입니다.');
            else if (error.response?.status === 401) alert('로그인 후 이용해주세요.');
            else alert(error.response?.data?.detail || '신고 접수에 실패했습니다.');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="report-modal-backdrop" onClick={onClose} role="presentation">
            <form className="report-modal" onSubmit={submit} onClick={(event) => event.stopPropagation()}>
                ...
                <select id="board-report-reason" value={reason} onChange={(event) => setReason(event.target.value)}>
                    <option>부적절한 내용</option><option>욕설·혐오 표현</option><option>개인정보 노출</option>
                    <option>광고·도배</option><option>기타</option>
                </select>
                <label htmlFor="board-report-detail">상세 내용 <span>(선택)</span></label>
                <textarea id="board-report-detail" value={detail} onChange={(event) => setDetail(event.target.value)}
