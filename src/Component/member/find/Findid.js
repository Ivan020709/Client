...
function FindId() {
    ...
    return (
        <div className="join-wrapper">
            ...
            {/* 성공했을 때만 나타나는 결과 */}
            {foundId ? (
                <div style={{ margin: '20px 0', padding: '15px', backgroundColor: '#f9f9f9', border: '1px solid #ddd', textAlign: 'center' }}>
                    <p style={{ margin: 0 }}>회원님의 아이디(이메일)는 다음과 같습니다:</p>
                    <p style={{ fontSize: '18px', color: 'blue', fontWeight: 'bold', marginTop: '10px' }}>{foundId}</p>
                    <button className="join-btn-join" style={{ marginTop: '15px' }} onClick={() => navigate('/Login')}>로그인으로 가기</button>
                </div>
            ) : (
